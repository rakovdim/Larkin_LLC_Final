 ...
 *   https://datatables.net/download/#bs/dt-1.10.10,b-1.1.0,rr-1.1.0,sc-1.4.0,se-1.1.0
 *
 * Included libraries:
 *   DataTables 1.10.10, Buttons 1.1.0, RowReorder 1.1.0, Scroller 1.4.0, Select 1.1.0
 */
...
(function (h) {
    ...
    m.ext = s = {
        buttons: {},
        classes: {},
        build: "bs/dt-1.10.10,b-1.1.0,rr-1.1.0,sc-1.4.0,se-1.1.0",
        errMode: "alert",
        feature: [],
        search: [],
...
(function (e) {
    ...
    var h = e.fn.dataTable, i = function (b, a) {
        ...
        this.dom = {clone: null};
        var d = this.s.dt.settings()[0], c = d.rowreorder;
        if (c)return c;
        d.rowreorder = this;
        this._constructor()
    };
    e.extend(i.prototype, {
        _constructor: function () {
            var b = this, a = this.s.dt, d = e(a.table().node());
            "static" === d.css("position") && d.css("position",
                "relative");
            e(a.table().container()).on("mousedown.rowReorder touchstart.rowReorder", this.c.selector, function (c) {
                var d = e(this).closest("tr");
                if (a.row(d).any())return b._mouseDown(c, d), !1
            });
            a.on("destroy.rowReorder", function () {
                e(a.table().container()).off(".rowReorder");
                a.off(".rowReorder")
            })
        }, _cachePositions: function () {
            var b = this.s.dt, a = e(b.table().node()).find("thead").outerHeight(), d = e.unique(b.rows({page: "current"}).nodes().toArray()), c = e.map(d, function (b) {
                return e(b).position().top - a
            }), d = e.map(c,
                function (a, d) {
                    return c.length < d - 1 ? (a + c[d + 1]) / 2 : (a + a + e(b.row(":last-child").node()).outerHeight()) / 2
                });
            this.s.middles = d;
            this.s.bodyTop = e(b.table().body()).offset().top;
            this.s.windowHeight = e(f).height()
        }, _clone: function (b) {
            var a = e(this.s.dt.table().node().cloneNode(!1)).addClass("dt-rowReorder-float").append("<tbody/>").append(b.clone(!1)), d = b.outerWidth(), c = b.outerHeight(), g = b.children().map(function () {
                return e(this).width()
            });
            a.width(d).height(c).find("tr").children().each(function (a) {
                this.style.width =
                    g[a] + "px"
            });
            a.appendTo("body");
            this.dom.clone = a
        }, _clonePosition: function (b) {
            var a = this.s.start, d = this._eventToPage(b, "Y") - a.top, b = this._eventToPage(b, "X") - a.left, c = this.c.snapX;
            this.dom.clone.css({
                top: d + a.offsetTop,
                left: !0 === c ? a.offsetLeft : "number" === typeof c ? a.offsetLeft + c : b + a.offsetLeft
            })
        }, _emitEvent: function (b, a) {
            this.s.dt.iterator("table", function (d) {
                e(d.nTable).triggerHandler(b + ".dt", a)
            })
        }, _eventToPage: function (b, a) {
            return -1 !== b.type.indexOf("touch") ? b.originalEvent.touches[0]["page" + a] : b["page" +
            a]
        }, _mouseDown: function (b, a) {
            var d = this, c = this.s.dt, k = this.s.start, j = a.offset();
            k.top = this._eventToPage(b, "Y");
            k.left = this._eventToPage(b, "X");
            k.offsetTop = j.top;
            k.offsetLeft = j.left;
            k.nodes = e.unique(c.rows({page: "current"}).nodes().toArray());
            this._cachePositions();
            this._clone(a);
            this._clonePosition(b);
            this.dom.target = a;
            a.addClass("dt-rowReorder-moving");
            e(g).on("mouseup.rowReorder touchend.rowReorder", function (a) {
                d._mouseUp(a)
            ...
            this._clonePosition(b);
            for (var a = this._eventToPage(b, "Y") - this.s.bodyTop, d = this.s.middles, c = null, k = this.s.dt, j = k.table().body(), f = 0, i = d.length; f < i; f++)if (a < d[f]) {
                c = f;
                break
            }
            null === c && (c = d.length);
            if (null === this.s.lastInsert || this.s.lastInsert !== c)0 === c ? this.dom.target.prependTo(j) : (a = e.unique(k.rows({page: "current"}).nodes().toArray()), c > this.s.lastInsert ? this.dom.target.before(a[c - 1]) : this.dom.target.after(a[c])),
                this._cachePositions(), this.s.lastInsert = c;
            b = this._eventToPage(b, "Y") - g.body.scrollTop;
            c = this.s.scrollInterval;
            65 > b ? c || (this.s.scrollInterval = setInterval(function () {
                g.body.scrollTop = g.body.scrollTop - 5
            }, 15)) : 65 > this.s.windowHeight - b ? c || (this.s.scrollInterval = setInterval(function () {
                g.body.scrollTop = g.body.scrollTop + 5
            }, 15)) : (clearInterval(c), this.s.scrollInterval = null)
        }, _mouseUp: function () {
            var b = this.s.dt, a, d, c = this.c.dataSrc;
            ...
            var f = this.s.start.nodes, j = e.unique(b.rows({page: "current"}).nodes().toArray()), i = {}, h = [], l = [], m = this.s.getDataFn, p = this.s.setDataFn;
            a = 0;
            for (d = f.length; a < d; a++)if (f[a] !== j[a]) {
                var n = b.row(j[a]).id(), q = b.row(j[a]).data(), o = b.row(f[a]).data();
                n && (i[n] = m(o));
                h.push({node: j[a], oldData: m(q), newData: m(o), newPosition: a, oldPosition: e.inArray(j[a], f)});
                l.push(j[a])
            }
            this._emitEvent("row-reorder",
                [h, {dataSrc: c, nodes: l, values: i, triggerRow: b.row(this.dom.target)}]);
            this.c.editor && this.c.editor.edit(l, !1, {submit: "changed"}).multiSet(c, i).submit();
            if (this.c.update) {
                a = 0;
                for (d = h.length; a < d; a++)f = b.row(h[a].node).data(), p(f, h[a].newData), b.columns().every(function () {
                    this.dataSrc() === c && b.cell(h[a].node, this.index()).invalidate("data")
                });
                b.draw(!1)
            }
        }
    });
    i.defaults = {dataSrc: 0, editor: null, selector: "td:first-child", snapX: !1, update: !0};
    i.version = "1.1.0";
    e.fn.dataTable.RowReorder = i;
    e.fn.DataTable.RowReorder =
        i;
    e(g).on("init.dt.dtr", function (b, a) {
        if ("dt" === b.namespace) {
            var d = a.oInit.rowReorder, c = h.defaults.rowReorder;
            if (d || c)c = e.extend({}, d, c), !1 !== d && new i(a, c)
        }
    });
    return i
});
...
(function (e) {
    ...
    e.extend(h.prototype, {
        fnRowToPixels: function (a,
            ...
            var c = this, d = !1, f = this.fnRowToPixels(a), j = a - (this.s.displayBuffer - 1) / 2 * this.s.viewportRows;
            0 > j && (j = 0);
            if ((f > this.s.redrawBottom ||
                f < this.s.redrawTop) && this.s.dt._iDisplayStart !== j)d = !0, f = this.fnRowToPixels(a, !1, !0);
            ...
            (a ===
            l || a) && this.s.dt.oInstance.fnDraw(!1)
            ...
            if (this.s.dt.oFeatures.bPaginate) {
                ...
                e(this.s.dt.nTable).on("init.dt", function () {
                    a.fnMeasure()
                });
            ...
            if (!this.s.skip && !this.s.ingnoreScroll)if (this.s.dt.bFiltered || this.s.dt.bSorted)this.s.lastScrollTop = 0; else {
                ...
                if (c < this.s.redrawTop || c >
                    ...
                    if (d != this.s.dt._iDisplayStart &&
                        (this.s.tableTop = e(this.s.dt.nTable).offset().top, this.s.tableBottom = e(this.s.dt.nTable).height() + this.s.tableTop, b = function () {
            ...
            var e = (c.scroll - c.viewport) / 2, j = (c.virtual - c.viewport) / 2;
            d = j / (e * e);
            ...
            var a = this, b = this.s.heights, c = this.dom.scroller.scrollTop, d = e(this.s.dt.nTable).height(), f = this.s.dt._iDisplayStart, j = this.s.dt._iDisplayLength, h = this.s.dt.fnRecordsDisplay();
            this.s.skip = !0;
            this._fnScrollForce();
            c = 0 === f ? this.s.topRowFloat * b.row : f + j >= h ? b.scroll - (h - this.s.topRowFloat) * b.row : this._domain("virtualToPhysical", this.s.topRowFloat * b.row);
            this.dom.scroller.scrollTop = c;
            this.s.baseScrollTop = c;
            this.s.baseRowTop = this.s.topRowFloat;
            var g = c - (this.s.topRowFloat - f) * b.row;
            0 === f ? g = 0 : f + j >= h && (g = b.scroll - d);
            ...
            var a = this.s.dt, b = a.nTable, c = b.cloneNode(!1), d = e("<tbody/>").appendTo(c), f = e('<div class="' + a.oClasses.sWrapper + ' DTS"><div class="' +
                a.oClasses.sScrollWrapper + '"><div class="' + a.oClasses.sScrollBody + '"></div></div></div>');
            for (e("tbody tr:lt(4)", b).clone().appendTo(d); 3 > e("tr", d).length;)d.append("<tr><td>&nbsp;</td></tr>");
            e("div." + a.oClasses.sScrollBody, f).append(c);
            f.appendTo(this.s.dt.nHolding || b.parentNode);
            this.s.heights.row = e("tr", d).eq(1).outerHeight();
            f.remove()
        }, _fnInfo: function () {
            if (this.s.dt.oFeatures.bInfo) {
                var a = this.s.dt, b = a.oLanguage, c = this.dom.scroller.scrollTop, d = Math.floor(this.fnPixelsToRow(c, !1, this.s.ani) +
                    ...
                    k).replace("_TOTAL_", l) + b.sInfoPostFix : b.sInfo.replace("_START_", g).replace("_END_", i).replace("_MAX_", k).replace("_TOTAL_", l) + " " + b.sInfoFiltered.replace("_MAX_", a.fnFormatNumber(a.fnRecordsTotal())) + b.sInfoPostFix;
...
(function (e) {
    ...
    function u(c) {
        ...
        if (a._select.info && a.aanFeatures.i) {
            var b =
                ...
                b.append(e('<span class="select-item"/>').append(c.i18n("select." + a + "s", {
                    _: "%d " + a + "s selected",
                    "0": "",
                    1: "1 " + a + " selected"
                }, d)))
    ...
    e.each([{type: "row", prop: "aoData"}, {type: "column", prop: "aoColumns"}], function (c, a) {
        g.ext.selector[a.type].push(function (b,
                                              d, c) {
    ...
    e.extend(g.ext.buttons, {
        selected: {
            text: o("selected", "Selected"), className: "buttons-selected", init: function (c) {
                var a = this;
    ...
    e.each(["Row", "Column", "Cell"], function (c, a) {
        ...
        g.ext.buttons["select" + a + "s"] = {
            ...
            init: function (a) {
                var c = this;
                a.on("selectItems.dt.DT", function (a,
                                                    d, e) {
                    c.active(e === b)
                })
            }
